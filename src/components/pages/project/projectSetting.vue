<template>
    <div class="grid">
        <content-box>
            ...
            <content-box-block
                class="q-mb-xl"
                :title="$t('projectSetting.description')"
                :star="'*'"
            >
                <q-input
                    type="textarea"
            ...
            <content-box-block
                class="q-mb-xl"
                :title="$t('projectSetting.tag.title')"
                :star="'*'"
            >
                <q-select
                    ref="hashtagsArr"
            ...
            <content-box-block
                class="q-mb-xl"
                :title="$t('projectSetting.thumbnailImg.title')"
                :star="'*'"
            >
                <content-box-block-image-uploader
                    ...
                    "
                    :text="$t('projectSetting.thumbnailImg.text')"
                    limit-size="4"
                >
                </content-box-block-image-uploader>
            ...
            <content-box-block
                class="q-mb-xl"
                :title="$t('projectSetting.previewImg.title')"
            >
            ...
            <content-box-block
                class="q-mb-xl"
                :title="$t('projectSetting.gamePath')"
            >
                <q-input input-class="input" v-model="gamePath" readonly />
            </content-box-block>
            <content-box-block
                class="q-mb-xl"
                :title="$t('projectSetting.gameId')"
            >
                <q-input input-class="input" v-model="gameId" readonly />
            </content-box-block>
            <content-box-line></content-box-line>
            <content-box-block
                :title="$t('projectSetting.delete.title')"
                class="q-mb-xl"
            >
                <div class="hintText">
                ...
                <div class="text-left">
                    <q-btn class="delete-btn" @click="deleteProject">
                        {{ $t("projectSetting.delete.btn") }}</q-btn
                    >
                </div>
            </content-box-block>
        </content-box>
        ...
        <content-box class="save-btn">
            <q-btn :loading="wait" color="primary" @click="save">{{
                $t("save")
            }}</q-btn>
        </content-box>
    </div>
...
export default class ProjectSetting extends Vue {
    ...
    async mounted() {
        ...
        this.gamePath =
            (project && project.game && project.game.pathname) || "";
    ...
    async save() {
        ...
        if (!result || result.error) {
            if (result.error.code === 40101) {
                ...
                Notify.create({
                    message: this.$t(
                        "projectSetting.error.saveEditGameFail"
                    ).toString(),
                    position: "top",
                    color: "negative",
                    timeout: 2000,
    ...
    async deleteProject() {
        ...
        if (ok) {
            ...
            if (!result || result.error) {
                Notify.create({
                    message: this.$t(
                        "projectSetting.error.deleteGameFail"
                    ).toString(),
                    position: "top",
                    color: "negative",
                    timeout: 2000,
    ...
    createValue(val, done) {
        ...
        if (this.hashtagsArr.length <= 20) {
            ...
            this.hashtagsError = this.$t(
                "projectSetting.error.tooManyInputs"
            ).toString();
        }
    }
