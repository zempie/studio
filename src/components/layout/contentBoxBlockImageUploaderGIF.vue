<template>
    <div class="uploader">
        <content-box-block-image
            v-if="url || defaultSrc"
            :src="url || defaultSrc"
        ></content-box-block-image>
        <div class="btn_wrapper">
            <content-box-block-upload-button
                :text="text"
                v-on:@click="fileLoader.addFile()"
            ></content-box-block-upload-button>
            <!-- 삭제 버튼 -->
            <template v-if="url || defaultSrc">
                <q-btn
                    color="grey-9 q-mb-sm"
                    class="removeBtn"
                    @click="reset()"
                >
                    <q-icon
...
export default class ContentBoxBlockImageUploaderGIF extends Vue {
    ...
    private url: string = localStorage.getItem("thumbFile2Url");
    ...
    onLoadFile(data, file: File) {
        if (this.limitSize && file.size > mbToByte(this.limitSize)) {
            alert(
                this.$t("contentBox.limitSizeErr") + ` ${this.limitSize}MB) `
            );
            return;
        }
